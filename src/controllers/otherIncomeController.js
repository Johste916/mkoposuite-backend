...
const searchWhere = (m, q) => {
  if (!q) return {};
  const fields = ['source', 'note', 'reference'].filter(f => m.rawAttributes?.[f]);
  return fields.length ? { [Op.or]: fields.map(f => ({ [f]: { [Op.iLike]: `%${q}%` } })) } : {};
};

exports.list = async (req, res) => {
  const Model = getModel('OtherIncome');
  const page = Math.max(parseInt(req.query.page || '1', 10), 1);
  const limit = Math.min(Math.max(parseInt(req.query.limit || '20', 10), 1), 200);
  const offset = (page - 1) * limit;
  const where = searchWhere(Model, req.query.q);
  const { rows, count } = await Model.findAndCountAll({ where, limit, offset, order: [['date', 'DESC']] });
