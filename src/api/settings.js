...
const base = (() => {
  const b = (api.defaults && api.defaults.baseURL) ? api.defaults.baseURL : "";
  return b && b.endsWith("/api") ? "" : "/api";
})();

const P = (p) => `${base}/settings/${p}`;
const A = (p) => `${base}/admin/${p}`;

export const SettingsAPI = {
  // ----- Entity lists / CRUD already existing -----
  ...
  // New: generic Admin Types & Templates helpers (used by the small pages above)
  listTypes: (category) => api.get(A(`types/${category}`)).then(r=>r.data),
  addType: (category, payload) => api.post(A(`types/${category}`), payload).then(r=>r.data),
  deleteType: (category, id) => api.delete(A(`types/${category}/${id}`)).then(r=>r.data),

  listTemplates: (category) => api.get(A(`templates/${category}`)).then(r=>r.data),
  addTemplate: (category, payload) => api.post(A(`templates/${category}`), payload).then(r=>r.data),
  deleteTemplate: (category, id) => api.delete(A(`templates/${category}/${id}`)).then(r=>r.data),
};
