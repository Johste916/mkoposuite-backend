// src/api/settings.js (helper)
import api from "./index";

// If baseURL already ends with /api, don't prepend /api again
const baseHasApi = typeof api?.defaults?.baseURL === "string" && /\/api\/?$/.test(api.defaults.baseURL || "");
const P = (p) => (baseHasApi ? `/settings/${p}` : `/api/settings/${p}`);
