...
module.exports = (sequelize, DataTypes) => {
  ...
  CashTransaction.afterCreate(async (tx, options) => {
    if (tx.status === 'void') return;
    const sign = (tx.direction === 'out' ? -1 : 1);

    try {
      await sequelize.models.CashAccount.update(
        { currentBalance: sequelize.literal(`"currentBalance" + (${sign} * ${Number(tx.amount)})`) },
        { where: { id: tx.cashAccountId }, transaction: options?.transaction }
      );
    } catch {}
    ...
    if (tx.type === 'loan_repayment' && tx.loanId) {
      ...
      if (LoanPayment) {
        ...
        if (has(LoanRepayment, 'method'))   payload.method = 'cash';
        if (has(LoanRepayment, 'reference'))payload.reference = tx.reference || `CASH:${tx.id}`;
