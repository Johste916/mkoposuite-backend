...
const NAMES_TO_MARK_DONE = [
  // Duplicates that already exist via .js.js versions:
  ...
  // The one that previously hung:
  '20250822071454-add-repayment-columns.js',

  // ❗ New: the one that now fails (column already exists)
  '20250822081527-add-status-to-loan-payments.js',
];

(async () => {
  try {
    for (const name of NAMES_TO_MARK_DONE) {
    ...
    console.log('✅ Marked selected migrations as done in SequelizeMeta');
  } catch (e) {
