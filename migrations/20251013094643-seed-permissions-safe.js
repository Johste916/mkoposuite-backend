...
/**
 * This seeds your full permission catalog.
 * - Uses snake_case timestamps (created_at/updated_at)
 * - Uses roles as text[] ('{}'::text[])
 * - No external require; catalog embedded here
 * - Idempotent via ON CONFLICT (action) DO NOTHING
 */
...
module.exports = {
      ...
      for (const a of g.actions) {
        await qi.sequelize.query(
          `INSERT INTO permissions (action, description, roles, is_system, created_at, updated_at)
           VALUES (:action, :desc, '{}'::text[], TRUE, NOW(), NOW())
  ...
  async down(queryInterface /*, Sequelize */) {
    ...
    if (actions.length) {
      await queryInterface.bulkDelete("permissions", { action: actions }, {}).catch(() => {});
    }
  },
};
