...
module.exports = {
  async up(qi, Sequelize) {
    ...
    const [exists] = await qi.sequelize.query(
      `SELECT to_regclass('public.collection_sheets') AS t;`
    );
    if (exists?.[0]?.t) return;

    // Helper to map detected PK type
    const mapType = (t) => {
      const s = String(t || '').toLowerCase();
      if (s.includes('uuid')) return Sequelize.UUID;
      if (s.includes('bigint')) return Sequelize.BIGINT;
      return Sequelize.INTEGER; // default/fallback
    };

    // Detect related tables and PK types (don’t fail if they’re missing)
    let hasBranches = true, hasUsers = true;
    let branchPkType = Sequelize.INTEGER, userPkType = Sequelize.INTEGER;

    try {
      const branches = await qi.describeTable('branches');
      branchPkType = mapType(branches?.id?.type);
    } catch { hasBranches = false; }
    ...
    // Make sure enum type exists (safe if already present)
    ...
    // Build FK columns conditionally (only add FK refs if target table exists)
