  ...
  async up(queryInterface, Sequelize) {
    ...
    if (!desc) {
      await queryInterface.createTable(table, {
        ...
        bank_ref:      { type: Sequelize.STRING(120), allowNull: true },
        ...
        reconciled_by: { type: Sequelize.UUID, allowNull: true },
      ...
      await ensure('bank_id',       { type: Sequelize.UUID, allowNull: false });
      ...
      await ensure('bank_ref',      { type: Sequelize.STRING(120), allowNull: true });
      await ensure('description',   { type: Sequelize.TEXT, allowNull: true });
      await ensure('note',          { type: Sequelize.TEXT, allowNull: true });
      await ensure('reconciled',    { type: Sequelize.BOOLEAN, allowNull: false, defaultValue: false });
      await ensure('reconciled_at', { type: Sequelize.DATE, allowNull: true });
      await ensure('reconciled_by', { type: Sequelize.UUID, allowNull: true });
    ...
    await addIdx(['tenant_id'], 'bank_tx_tenant_id');
    await addIdx(['bank_id'], 'bank_tx_bank_id');
    await addIdx(['tenant_id','bank_id','status'], 'bank_tx_tenant_id_bank_id_status');
    await addIdx(['tenant_id','type'], 'bank_tx_tenant_id_type');
    await addIdx(['occurred_at'], 'bank_tx_occurred_at');
    await addIdx(['reconciled'], 'bank_tx_reconciled');
  },

  async down(queryInterface) {
    const table = { tableName: 'bank_transactions', schema: 'public' };
    const drop = async (name) => { try { await queryInterface.removeIndex(table, name); } catch (_) {} };
    await drop('bank_tx_tenant_id');
    await drop('bank_tx_bank_id');
    await drop('bank_tx_tenant_id_bank_id_status');
    await drop('bank_tx_tenant_id_type');
    await drop('bank_tx_occurred_at');
    await drop('bank_tx_reconciled');
    try { await queryInterface.dropTable(table); } catch (_) {}
    try { await queryInterface.sequelize.query('DROP TYPE IF EXISTS "enum_public_bank_transactions_direction";'); } catch (_) {}
  }
};
